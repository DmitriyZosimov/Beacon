...
@Repository
public interface MobileSearchDao extends JpaRepository<Mobile, String> {
    ...
    @Query(nativeQuery = true,
            value = "with request as (select (concat('%', arr, '%')) from unnest(string_to_array(lower(:query), ' ')) as arr) " +
                    "select *, 0 as clazz_ " +
                    "from (" +
                    "   select * from mobile m where lower(m.brand) like any (select * from request) " +
                    "   union all " +
                    "   select * from mobile m where lower(m.model) like any (select * from request) " +
                    "   union all " +
                    "   select * from mobile m where lower(m.color) like any (select * from request) " +
                    "   union all " +
                    "   select * from mobile m where concat(m.ram, m.storage_capacity, m.battery) like any " +
                    "(select * from request) " +
                    "   union all " +
                    "   select * from mobile m where lower(m.os) like any (select * from request) " +
                    "   union all " +
                    "   select * from mobile m where lower(m.chipset_model) like any (select * from request) " +
                    "   union all " +
                    "   select * from mobile m where lower(m.camera_resolution) like any (select * from request) " +
                    "   ) as ir " +
